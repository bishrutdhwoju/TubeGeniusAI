...
import { PlusCircle, FileAudio, Video, Trash2, History } from 'lucide-react';
import { ProjectStatus, WorkflowType } from '../types';

export const Sidebar: React.FC = () => {
  const { projects, activeProjectId, setActiveProjectId, deleteProject } = useApp();

  return (
    <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col h-screen text-slate-300">
      <div className="p-4 border-b border-slate-800">
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <span className="w-8 h-8 bg-brand-600 rounded-lg flex items-center justify-center text-white text-lg">T</span>
          TubeGenius
        </h1>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="space-y-1 overflow-y-auto max-h-[calc(100vh-200px)]">
          {projects.length === 0 && (
            <p className="text-sm text-slate-600 px-2 italic">No projects yet.</p>
          )}
          ...
          {projects.map((project) => (
            <div
              ...
              className={`group flex items-center justify-between p-2 rounded-md cursor-pointer transition-colors ${
                activeProjectId === project.id ? 'bg-slate-800 text-brand-400' : 'hover:bg-slate-800/50'
              }`}
              onClick={() => setActiveProjectId(project.id)}
            >
              <div className="flex items-center gap-3 overflow-hidden">
                {project.type === WorkflowType.TOPIC_TO_VIDEO ? (
                  <Video size={16} className={activeProjectId === project.id ? 'text-brand-400' : 'text-slate-500'} />
                ) : (
                  <FileAudio size={16} className={activeProjectId === project.id ? 'text-brand-400' : 'text-slate-500'} />
                )}

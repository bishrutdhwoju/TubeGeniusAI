...
export const ProjectView: React.FC<ProjectViewProps> = ({ project }) => {
  ...
  const [selectedVoice, setSelectedVoice] = useState<VoiceName>(project.selectedVoice);
  ...
  const handleRegenerateAudio = () => {
    regenerateAudio(project.id, selectedVoice);
  };
  ...
  const isGenerating = project.status === ProjectStatus.GENERATING_AUDIO || project.status === ProjectStatus.GENERATING_SCRIPT;
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 overflow-hidden">
      ...
      <header className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-900/50 backdrop-blur-sm z-10">
        <div>
           <h2 className="text-lg font-semibold text-white truncate max-w-md">{project.name}</h2>
           <div className="flex items-center gap-2 text-xs text-slate-400">
             <span className="bg-slate-800 px-2 py-0.5 rounded text-slate-300">{project.type === WorkflowType.TOPIC_TO_VIDEO ? 'SEO Script Mode' : 'Voiceover Mode'}</span>
             {isGenerating && <span className="text-brand-400 animate-pulse flex items-center gap-1"><RefreshCw size={10} className="animate-spin" /> Processing...</span>}
           </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 relative bg-slate-950">
             {project.status === ProjectStatus.GENERATING_SCRIPT ? (
               <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500">
                 <Bot size={48} className="animate-bounce mb-4 text-brand-500 opacity-50" />
        ...
        <div className="w-96 flex flex-col bg-slate-900 border-l border-slate-800 overflow-y-auto">
          ...
          <div className="p-6 border-b border-slate-800">
            ...
            {/* Voice Selector */}
            <div className="mb-4">
              <label className="block text-xs text-slate-500 mb-2 uppercase tracking-wide font-semibold">Voice Persona</label>
            ...
            {project.audioUrl ? (
                ...
                <div className="h-32 bg-slate-800/50 rounded-lg border-2 border-dashed border-slate-700 flex flex-col items-center justify-center text-slate-500 p-4 text-center">
                    {project.status === ProjectStatus.GENERATING_AUDIO ? (
                        <>
                            <RefreshCw size={24} className="animate-spin mb-2 text-brand-500" />
          ...
          {project.type === WorkflowType.TOPIC_TO_VIDEO && project.seoMetadata && (
            <div className="p-6">
               <h3 className="text-sm font-bold text-slate-200 mb-4">SEO Metadata</h3>
               
               <div className="space-y-4">
                 <div>
                    <label className="text-xs text-slate-500 font-semibold uppercase">Title</label>
                    <p className="text-sm text-slate-300 bg-slate-950 p-2 rounded border border-slate-800 mt-1 select-all">{project.seoMetadata.title}</p>
                 </div>
                 
                 <div>
                    <label className="text-xs text-slate-500 font-semibold uppercase">Description</label>
                    <p className="text-xs text-slate-400 bg-slate-950 p-2 rounded border border-slate-800 mt-1 h-24 overflow-y-auto select-all whitespace-pre-wrap">{project.seoMetadata.description}</p>
                 </div>

                 <div>
                    <label className="text-xs text-slate-500 font-semibold uppercase">Tags</label>
                    <div className="flex flex-wrap gap-1 mt-1">
                        {project.seoMetadata.tags.map(tag => (
                            <span key={tag} className="text-[10px] bg-slate-800 text-slate-300 px-2 py-1 rounded border border-slate-700">#{tag}</span>
                        ))}
                    </div>
                 </div>

                 <div>
                    <label className="text-xs text-slate-500 font-semibold uppercase">Pinned Comment</label>
                    <p className="text-xs text-slate-400 bg-slate-950 p-2 rounded border border-slate-800 mt-1 select-all">{project.seoMetadata.pinnedComment}</p>
                 </div>
               </div>
            </div>
          )}
